...
namespace GrouperLib.Store
{
    ...
    public class OnPremAd : IMemberSource, IGroupStore
    {
        ...
        private readonly GroupMemberSource[] _supportedGroupMemberSources = new GroupMemberSource[]  {
            GroupMemberSource.OnPremAdGroup,
            GroupMemberSource.OnPremAdQuery
        };
        private readonly GroupStore[] _supportedGroupStores = new GroupStore[] { GroupStore.OnPremAd };
        private readonly TimeSpan _cacheExpiration = TimeSpan.FromMinutes(10);
        private readonly string[] _memberAttributes = new string[] { ObjectGuidAttribute, UserPrincipalNameAttribute, CommonNameAttribute };
        private readonly string[] _groupInfoAttributes = new string[] { DisplayNameAttribute, CommonNameAttribute };
        ...
        private async Task<string?> GetDistinguishedNameAsync(Guid objectId)
        {
            ...
            await foreach (var obj in _ldap.SearchObjectsAsync(Ldap.GetObjectGuidFilter(objectId), new string[] { DistinguishedNameAttribute }))
            {
